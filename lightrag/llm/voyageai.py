...
_VOYAGEAI_IMPORT_ERROR: Exception | None = None
...
try:
    ...
    # Keep this module importable in environments where optional provider
    # dependencies are absent or temporarily incompatible with the Python
    # runtime. The actual SDK is only required when voyageai_embed is called;
    # tests can still patch voyageai.AsyncClient on this placeholder.
    _VOYAGEAI_IMPORT_ERROR = e
    voyageai = SimpleNamespace(AsyncClient=None)
    APIConnectionError = RateLimitError = ServerError = ServiceUnavailableError = (
        Timeout
    ) = TryAgain = VoyageAIError
...
async def voyageai_embed(
    ...
    try:
        if getattr(voyageai, "AsyncClient", None) is None:
            raise ImportError(
                "VoyageAI SDK is not available. Install a voyageai version "
                "compatible with this Python runtime."
            ) from _VOYAGEAI_IMPORT_ERROR
