...
- This repo is a fork of `HKUDS/LightRAG`. Default fork flow uses only `main` and `dev`: do all integration and implementation work on `dev`, then open PRs from `dev` to the fork's `main`.
- Do not create extra feature or merge branches for routine work unless the user explicitly asks for one or a risky experiment needs isolation. Upstream PRs to `HKUDS/LightRAG:main` are only for intentional contribution back to the original project.
- If the CLI sandbox blocks writes under `.git/refs`, request escalation for branch creation or other ref updates instead of retrying blindly.
